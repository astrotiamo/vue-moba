module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = ["9月15日全服不停机更新公告", "孙尚香-蔷薇恋人优化后海报过程稿&amp;亚瑟-心灵脉冲局内外模型优化【老亚瑟答疑】", "狄某有话说丨张良：两耳不闻峡谷事，一心只读魔法书", "甄姬-冰雪圆舞曲 优化进展【老亚瑟的答疑时间】", "9月7日抢先服版本更新公告", "孙尚香-蔷薇恋人优化后海报过程稿&amp;亚瑟-心灵脉冲局内外模型优化【老亚瑟答疑】", "《天天酷跑》8岁生日快乐，8重壕礼齐贺庆典！", "QQ名人赛第二季第四期来啦！“hiphop lover”奥斯卡遇上“电竞女神”", "狄某有话说丨张良：两耳不闻峡谷事，一心只读魔法书", "甄姬-冰雪圆舞曲 优化进展【老亚瑟的答疑时间】", "9月15日体验服停机更新公告", "9月15日净化游戏环境声明及处罚公告", "9月15日游戏内违规签名、攻略信息处罚公告", "9月15日外挂专项打击公告", "9月15日“演员”惩罚名单", "佳期胜赏正中秋，峡谷好礼欢乐游", "抢先服新赛季更新，弈星重塑登场", "【王者密令 送荣耀积分】活动公告", "嫦娥源·梦皮肤场景票选结果公布", "重庆QGhappy夺得世冠总冠军，多重回馈福利来相送", "安徽赛区JXG战队获得“全民全运”王者荣耀“广电网络 秦岭云”杯冠军！", "【决赛Day2预告】登顶王座，折得桂冠!", "【决赛阶段Day1战报1】：“全运”决赛正式打响，八强集结剑指桂冠", "八强云集，决战西安!“全民全运”王者荣耀“广电网络秦岭云”杯决赛火热开启", "【全民全运王者系列赛事：决赛Day1预告】壮志凌云，决战将至!"]
  ...
  router.get('/news/list', async(req, res) => {
    ...
    const subCats = cats.map(v => v._id) // 4个分类id的所有文档
    cats.unshift({
      name: '热门',
      newsList: await Article.find().where({
        categories: {$in: subCats}
      }).populate('categories').limit(5).lean() // 在文章集合里找出categories等于4个分类的id的所有文档
    })
  ...
  router.get('/heroes/list', async(req, res) => {
    ...
    // console.log(cats);
    const subCats = cats.map(v => v._id) // 6个英雄分类id的所有英雄
    cats.unshift({
      name: '热门',
      heroList: await Hero.find().where({
        categories: {$in: subCats}
      }).limit(10).lean() // 在文章集合里找出categories等于6个英雄分类的id的所有英雄
    })
